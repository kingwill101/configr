...
void main() {
  group('apt block integration (container-backed)', () {
    late ContainerTestHelper container;
    ...
    setUpAll(() async {
      ...
      final pubGetResult =
          await container.runCommand('cd /app && dart pub get 2>&1');
      expect(pubGetResult.exitCode, 0,
          reason: 'dart pub get failed: ${pubGetResult.stdout}');

      final aptUpdateResult =
          await container.runCommand('apt-get update -qq 2>&1');
      expect(aptUpdateResult.exitCode, 0,
    ...
    test('configr apply installs hello package via apt block', () async {
...
''';
      ...
      try {
        ...
        final dpkgResult =
            await container.runCommand('dpkg -s hello 2>&1');
        expect(dpkgResult.exitCode, 0,
            reason: 'hello should be installed: ${dpkgResult.stdout}');

        final whichResult =
            await container.runCommand('which hello 2>&1');
        expect(whichResult.exitCode, 0,
    ...
    }, tags: debianTag, timeout: const Timeout(Duration(minutes: 3)));

    test('configr apply uninstalls hello package via apt block', () async {
...
''';
      ...
      try {
        ...
        final whichResult =
            await container.runCommand('which hello 2>&1 || true');
        expect(whichResult.stdout, isEmpty,
    ...
    test('configr apply reports error for non-existent package',
        () async {
      const configContent = '''
apt {
...
''';
      ...
      try {
        ...
        expect(result.stdout, contains('Unable to locate package'),
            reason: 'Should mention package not found in stdout: ${result.stdout}');
      } finally {
