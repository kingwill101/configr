  ...
  void loadModules() {
    for (final action in action.actions) {
      if (action.type == beforeHookType) {
        ...
        _beforeHooks.add(createHookModule(file, action, fileSystem));
      } else if (action.type == afterHookType) {
        // For after hooks, create a module that executes the nested actions
        _afterHooks.add(createHookModule(file, action, fileSystem));
...
ResourceModule createHookModule(
  ...
  // Create a special module that executes all nested actions in the hook
  return HookExecutionModule(file, hookAction, fileSystem: fs);
}
...
class HookExecutionModule extends ResourceModule {
  ...
  Future<void> execute() async {
    // Execute all nested actions in the hook directly
    for (final nestedAction in action.actions) {
      final module = getModuleForAction(file, nestedAction, fileSystem);
      await module();
    }
  }
}
...
ResourceModule createResourceModule(
  ...
  // Create a generic resource module that can handle all actions for a resource
  return GenericResourceModule(resource, fileSystem: fs);
}
...
class GenericResourceModule extends ResourceModule {
  ...
  @override
  Future<void> execute() async {
    // Execute all modules (hooks and main actions)
...
ResourceModule getModuleForAction(
  ...
  switch (action.type) {
    ...
    case 'file':
      return FileFileModule(file, action, fileSystem: fs);
    case 'git':
      return FileGitModule(file, action, fileSystem: fs);
    case 'network':
      return FileNetworkModule(file, action, fileSystem: fs);
    default:
