// dart:io removed - using file package for abstraction
...
class FileSymlinkModule extends ResourceModule {
  ...
  FileSymlinkModule(super.file, super.action,
      {super.allowedActions = const ['symlink'], super.fileSystem, super.eventBus}) {
    updateState({
  ...
  Future<bool> _isDirectoryOperation() async {
    return includePatterns.isNotEmpty || 
           (await FileUtils.pathExists(resolvePath(source), fileSystem: fileSystem)).isDir;
  }

  /// Execute single symlink operation
  Future<void> _executeSingleOperation() async {
    final sourcePath = resolvePath(source);
  ...
  Future<void> _collectFilesRecursively(String dirPath, List<String> files) async {
    ...
    final dir = (fileSystem ?? fs).directory(dirPath);
    await for (final entity in dir.list(recursive: true, followLinks: false)) {
      final file = (fileSystem ?? fs).file(entity.path);
    ...
    try {
      ...
      for (final dir in createdDirs) {
        if (await FileUtils.directoryExists(dir, fileSystem: fileSystem)) {
          ...
          final dirEntity = (fileSystem ?? fs).directory(dir);
