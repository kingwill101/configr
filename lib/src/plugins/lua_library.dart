...
class ConfigrLibrary extends Library {
  ...
  @override
  String get description => 'Built-in Configr plugin API functions (getEnv, '
  ...
  void registerFunctions(LibraryRegistrationContext context) {
    ...
    context.define('getEnv', builder.create((args) {
      return Platform.environment[_stringArg(args, 0)];
    }));
    context.describe('getEnv', FunctionDoc(
    ...
    context.define('setVariable', builder.create((args) {
    ...
    context.describe('setVariable', FunctionDoc(
    ...
    context.define('logInfo', builder.create((args) {
      logger.info(_stringArg(args, 0));
      return null;
    }));
    context.describe('logInfo', FunctionDoc(
      summary: 'Logs an info-level message to the Configr logger.',
      params: [
        DocParam('message', 'string', 'The message to log.'),
      ],
      returns: 'nil',
      category: 'logging',
    ...
    context.define('logWarning', builder.create((args) {
      logger.warning(_stringArg(args, 0));
      return null;
    }));
    context.describe('logWarning', FunctionDoc(
    ...
    context.define('logError', builder.create((args) {
      logger.severe(_stringArg(args, 0));
      return null;
    }));
    context.describe('logError', FunctionDoc(
    ...
    context.define('logDebug', builder.create((args) {
      logger.fine(_stringArg(args, 0));
      return null;
    }));
    context.describe('logDebug', FunctionDoc(
    ...
    context.define('fileExists', builder.create((args) {
      return _host.fileSystem.file(_stringArg(args, 0)).existsSync();
    }));
    context.describe('fileExists', FunctionDoc(
      summary: 'Checks whether a file exists at the given path.',
      params: [
        DocParam('path', 'string', 'Path to check.'),
      ],
      returns: 'boolean',
      category: 'filesystem',
    ...
    context.define('readFile', builder.create((args) {
      return _host.fileSystem.file(_stringArg(args, 0)).readAsStringSync();
    }));
    context.describe('readFile', FunctionDoc(
    ...
    context.define('writeFile', builder.create((args) {
    ...
    context.describe('writeFile', FunctionDoc(
    ...
    context.define('appendFile', builder.create((args) {
      if (args.length < 2) return null;
      _host.fileSystem.file(_stringArg(args, 0)).writeAsStringSync(
        _stringArg(args, 1),
        mode: FileMode.append,
      );
      return null;
    }));
    context.describe('appendFile', FunctionDoc(
    ...
    context.define('configrCacheDir', builder.create((args) {
      if (_host.currentContext == null) return '';
      return (_host.currentContext!.globalContext.options['_configrCacheDir']
          as String?) ??
          '';
    }));
    context.describe('configrCacheDir', FunctionDoc(
    ...
    context.define('configrBackupDir', builder.create((args) {
      if (_host.currentContext == null) return '';
      return (_host.currentContext!.globalContext.options['_configrBackupDir']
          as String?) ??
          '';
    }));
    context.describe('configrBackupDir', FunctionDoc(
    ...
    context.define('emitStatusUpdate', builder.create((args) {
    ...
    context.describe('emitStatusUpdate', FunctionDoc(
    ...
    context.define('registerBlock', builder.create((args) {
      if (args.length < 2) return null;
      _host.registerBlockInPlugin(_stringArg(args, 0), Value.wrap(args[1]));
      return null;
    }));
    context.describe('registerBlock', FunctionDoc(
  ...
  rollback = function(block)
    ...
    context.define('getContext', builder.create((args) {
      final key = _stringArg(args, 0);
      return PluginContext.create().toMap()[key];
    }));
    context.describe('getContext', FunctionDoc(
      summary:
    ...
    context.describeTable('context', TableDoc(
      name: 'context',
      ...
      fields: [
        ...
        FieldDoc(
          ...
          fields: [
            FieldDoc(key: 'username', type: 'string', description: 'Username.'),
            FieldDoc(key: 'home', type: 'string', description: 'Home directory.'),
            FieldDoc(key: 'shell', type: 'string', description: 'Default shell.'),
          ],
        ),
        FieldDoc(
          key: 'env',
    ...
    context.describeTable('block', TableDoc(
      name: 'block',
      description:
    ...
    context.describeTable('registerBlock.callbacks', TableDoc(
      ...
      fields: [
        FieldDoc(
          ...
          description: 'Required. Called when the block is executed. Receives a block table.',
          required: true,
        ),
        FieldDoc(
          key: 'rollback',
          type: 'fun(block: block): nil',
          description: 'Optional. Called when the block is rolled back. Receives a block table.',
        ),
        FieldDoc(
          key: 'commands',
          type: '{ [string]: fun(...) }',
