import 'package:artisanal/artisanal.dart' show Console, Verbosity;
import 'package:configr/src/events/module_events.dart';
import 'package:configr/src/cli/ui/handlers/base_handler.dart';

/// Simple text-focused CLI handler using artisanal [Console] for output.
  ...
  void handleEvent(ModuleEvent event) {
    ...
    switch (event.eventType) {
      case ModuleEventType.started:
        ...
        _console.info(
          '🔄 [${event.moduleId}] ${startedEvent.message}',
      ...
      case ModuleEventType.progress:
        ...
        if (progressEvent.total > 10) {
          ...
          _console.info(
            '📊 [${event.moduleId}] $bar $percentage% - ${progressEvent.message}',
      ...
      case ModuleEventType.completed:
        ...
        _console.success(
          '✅ [${event.moduleId}] ${completedEvent.message}',
          verbosity: Verbosity.normal,
        );
        break;
      case ModuleEventType.failed:
        ...
        _console.error(
          '❌ [${event.moduleId}] ${failedEvent.message}',
          verbosity: Verbosity.normal,
        );
        break;
      case ModuleEventType.statusUpdate:
        ...
        switch (statusEvent.level) {
          case StatusEvent.warning:
            _console.warn(
              '⚠️ [${event.moduleId}] ${statusEvent.message}',
              verbosity: Verbosity.verbose,
            );
          case StatusEvent.error:
            _console.error(
              '💥 [${event.moduleId}] ${statusEvent.message}',
              verbosity: Verbosity.normal,
            );
          case StatusEvent.debug:
            if (_debugMode) {
              _console.writeln('🔍 [${event.moduleId}] ${statusEvent.message}');
            }
          case StatusEvent.info:
            _console.info(
              'ℹ️ [${event.moduleId}] ${statusEvent.message}',
              verbosity: Verbosity.verbose,
            );
        }
        break;
      case ModuleEventType.downloadProgress:
        ...
        if (downloadEvent.total > 1024 * 1024) {
          ...
          final bar = _generateProgressBar(percentage);
          _console.info(
            '⬇️ [${event.moduleId}] $bar $percentage% - ${downloadEvent.message}',
            verbosity: Verbosity.verbose,
          );
        }
        break;
      case ModuleEventType.error:
        ...
        _console.error(
          '💥 [${event.moduleId}] ${errorEvent.message}',
          verbosity: Verbosity.normal,
        );
        break;
      case ModuleEventType.retry:
        ...
        _console.info(
          '🔄 [${event.moduleId}] Retrying ${retryEvent.operation} '
      ...
      case ModuleEventType.performance:
        ...
        _console.info(
          '⚡ [${event.moduleId}] ${perfEvent.operation} '
      ...
      case ModuleEventType.security:
        ...
        _console.writeln(
          '🔒 [${event.moduleId}] ${securityEvent.securityEventType} '
      ...
      case ModuleEventType.plugin:
        ...
        _console.info(
          '🔌 [${event.moduleId}] ${pluginEvent.pluginName} ${pluginEvent.action}',
          verbosity: Verbosity.verbose,
        );
        break;
      ...
      default:
        _console.writeln(
          '❓ [${event.moduleId}] ${event.eventType.name}: ${event.toString()}',
        );
    }
  }
