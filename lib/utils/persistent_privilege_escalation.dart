...
class PersistentPrivilegeLock {
  ...
  static PersistentPrivilegeLock get instance => _instance ??= PersistentPrivilegeLock._();
  ...
  Future<void> acquire({UIHandler? uiHandler}) async {
    ...
    _startTimeoutTimer();
    logger.info('Persistent privilege lock acquired with shell PID: ${_shellProcess?.pid}');
  }
  ...
  Future<ProcessResult> executeCommand(String command, List<String> arguments) async {
    ...
    final commandWithId = 'echo "CMD_START:$commandId" && $fullCommand && echo "CMD_END:$commandId"';
  ...
  /// Start a persistent sudo shell
  Future<void> _startPersistentShell({UIHandler? uiHandler}) async {
    try {
      ...
      _shellProcess = await Process.start('sudo', ['-i'], mode: ProcessStartMode.normal);
  ...
  Future<bool> _authenticateSudo({UIHandler? uiHandler}) async {
    ...
    if (uiHandler != null) {
      ...
      try {
        final password = uiHandler.promptPassword('Sudo password required for persistent privilege lock');
  ...
  void _startTimeoutTimer() {
    _timeoutTimer = Timer(_timeoutDuration, () {
      logger.info('Persistent privilege lock timed out after ${_timeoutDuration.inMinutes} minutes');
      release();
    });
  }
...
class PersistentSudoEscalation implements PrivilegeEscalation {
  ...
  PersistentSudoEscalation({this.uiHandler});
  ...
  Future<ProcessResult> runWithElevatedPrivileges(
    ...
    if (usePrivilegeLock && lock.isActive) {
      try {
        ...
        logger.warning('Failed to use persistent privilege lock, falling back to direct sudo: $e');
    ...
    if (usePrivilegeLock && !lock.isActive) {
      try {
        ...
        logger.warning('Failed to acquire persistent privilege lock, falling back to direct sudo: $e');
        // Fall through to direct sudo
      }
    }
  ...
  Future<ProcessResult> _runDirectSudo(String command, List<String> arguments) async {
    // Try passwordless sudo first
    ...
    if (uiHandler != null) {
      ...
      try {
        ...
        final fullCommand = 'echo "$password" | sudo -S $command ${arguments.join(' ')}';
        result = await Process.run('sh', ['-c', fullCommand]);
