...
How Zed was started â whether it's icon was clicked in the macOS Dock or in a Linux window manager, or whether it was started via the CLI `zed` that comes with Zed â influences which environment variables Zed can use.
...
- If the language servers was not found in the project environment, Zed tries to install it globally and start it globally. In that case, the process will inherit Zed's process environment, and â if the project was opened via ClI â from the CLI.
