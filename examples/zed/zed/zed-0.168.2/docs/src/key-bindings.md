...
Zed has a very customizable key binding system â you can tweak everything to work exactly how your fingers expect!
...
The keys can be any single unicode codepoint that your keyboard generates (for example `a`, `0`, `Â£` or `Ã§`), or any named key (`tab`, `f1`, `shift`, or `cmd`). If you are using a non-Latin layout (e.g. Cyrillic), you can bind either to the cyrillic character, or the latin character that that key generates with `cmd` pressed.

A few examples:

```json
 "bindings": {
   "cmd-k cmd-s": "zed::OpenKeymap", // matches â-k then â-s
   "space e": "editor::Complete", // type space then e
   "Ã§": "editor::Complete", // matches â¥-c
...
The `alt-` modifier can be used on many layouts to generate a different key. For example on macOS US keyboard the combination `alt-c` types `Ã§`. You can match against either in your keymap file, though by convention Zed spells this combination as `alt-c`.
