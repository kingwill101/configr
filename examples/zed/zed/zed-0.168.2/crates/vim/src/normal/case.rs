...
mod test {
    ...
    async fn test_change_case(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_shared_state("aðCÂ«dÃ1*fËÂ»\n").await;
        cx.simulate_shared_keystrokes("~").await;
        cx.shared_state().await.assert_eq("aðCËDÃ©1*F\n");

        // works with multibyte characters
        cx.simulate_shared_keystrokes("~").await;
        cx.set_shared_state("aËCðÃ©1*F\n").await;
        cx.simulate_shared_keystrokes("4 ~").await;
        cx.shared_state().await.assert_eq("acðÃ1Ë*F\n");
