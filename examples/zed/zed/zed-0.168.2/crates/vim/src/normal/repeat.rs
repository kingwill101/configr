...
mod test {
    ...
    async fn test_dot_repeat(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_shared_state("Ëthe quick brown fox").await;
        cx.simulate_shared_keystrokes("2 ~ .").await;
        cx.set_shared_state("THE Ëquick brown fox").await;
        cx.simulate_shared_keystrokes("3 .").await;
        cx.set_shared_state("THE QUIËck brown fox").await;
        cx.run_until_parked();
        cx.simulate_shared_keystrokes(".").await;
        cx.shared_state().await.assert_eq("THE QUICK Ëbrown fox");
    }
    ...
    async fn test_repeat_ime(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state("hËllo", Mode::Normal);
        cx.simulate_keystrokes("i");

        // simulate brazilian input for Ã¤.
        cx.update_editor(|editor, cx| {
            editor.replace_and_mark_text_in_range(None, "\"", Some(1..1), cx);
            editor.replace_text_in_range(None, "Ã¤", cx);
        });
        cx.simulate_keystrokes("escape");
        cx.assert_state("hËÃ¤llo", Mode::Normal);
        cx.simulate_keystrokes(".");
        cx.assert_state("hËÃ¤Ã¤llo", Mode::Normal);
    }
    ...
    async fn test_repeat_over_blur(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_shared_state("Ëhello hello hello\n").await;
        cx.simulate_shared_keystrokes("c f o x escape").await;
        cx.shared_state().await.assert_eq("Ëx hello hello\n");
        cx.simulate_shared_keystrokes(": escape").await;
        cx.simulate_shared_keystrokes(".").await;
        cx.shared_state().await.assert_eq("Ëx hello\n");
    }
    ...
    async fn test_undo_repeated_insert(cx: &mut gpui::TestAppContext) {
        ...
        cx.shared_state().await.assert_eq("hello..Ë.");
    ...
    async fn test_record_replay(cx: &mut gpui::TestAppContext) {
        ...
        cx.shared_state().await.assert_eq("Ëj world");
        cx.simulate_shared_keystrokes("2 l @ w").await;
        cx.shared_state().await.assert_eq("j Ëj");
    }
    ...
    async fn test_record_replay_count(cx: &mut gpui::TestAppContext) {
        ...
        cx.simulate_shared_keystrokes("q a v 3 l s 0 escape l q")
            .await;
        cx.shared_state().await.assert_eq("0Ëo world!!");
        cx.simulate_shared_keystrokes("2 @ a").await;
        cx.shared_state().await.assert_eq("000Ë!");
    }
    ...
    async fn test_record_replay_dot(cx: &mut gpui::TestAppContext) {
        ...
        cx.shared_state().await.assert_eq("abËllo world");
        cx.simulate_shared_keystrokes(".").await;
        cx.shared_state().await.assert_eq("abËblo world");
        cx.simulate_shared_keystrokes("shift-q").await;
        cx.shared_state().await.assert_eq("ababËo world");
        cx.simulate_shared_keystrokes(".").await;
        cx.shared_state().await.assert_eq("ababËb world");
    }
    ...
    async fn test_record_replay_of_dot(cx: &mut gpui::TestAppContext) {
        ...
        cx.shared_state().await.assert_eq("Ëoello world");
        cx.simulate_shared_keystrokes("d l").await;
        cx.shared_state().await.assert_eq("Ëello world");
        cx.simulate_shared_keystrokes("@ w").await;
        cx.shared_state().await.assert_eq("Ëllo world");
    }
    ...
    async fn test_record_replay_interleaved(cx: &mut gpui::TestAppContext) {
        ...
        cx.shared_state().await.assert_eq("aËello world");
        cx.simulate_shared_keystrokes("q b @ z @ z q").await;
        cx.shared_state().await.assert_eq("aaaËlo world");
        cx.simulate_shared_keystrokes("@ @").await;
        cx.shared_state().await.assert_eq("aaaaËo world");
        cx.simulate_shared_keystrokes("@ b").await;
        cx.shared_state().await.assert_eq("aaaaaaËworld");
        cx.simulate_shared_keystrokes("@ @").await;
        cx.shared_state().await.assert_eq("aaaaaaaËorld");
        cx.simulate_shared_keystrokes("q z r b l q").await;
        cx.shared_state().await.assert_eq("aaaaaaabËrld");
        cx.simulate_shared_keystrokes("@ b").await;
        cx.shared_state().await.assert_eq("aaaaaaabbbËd");
    }
}
