...
async fn test_wrapped_motions(cx: &mut gpui::TestAppContext) {
    ...
    cx.set_shared_state(indoc! {"
        ...
        wow
        123456789012ðððððð123456789012"
    })
    .await;
    cx.simulate_shared_keystrokes("j j").await;
    cx.shared_state().await.assert_eq(indoc! {"
        ...
        wow
        123456789012ððËðððð123456789012"
    });
}
...
async fn test_remap_adjacent_dog_cat(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|cx| {
        cx.bind_keys([
            KeyBinding::new(
                "d o g",
                workspace::SendKeystrokes("ð¶".to_string()),
                Some("vim_mode == insert"),
            ),
            KeyBinding::new(
                "c a t",
    ...
    cx.neovim.exec("imap dog ð¶").await;
    cx.neovim.exec("imap cat ð±").await;

    cx.set_shared_state("Ë").await;
    cx.simulate_shared_keystrokes("i d o g").await;
    cx.shared_state().await.assert_eq("ð¶Ë");

    cx.set_shared_state("Ë").await;
    cx.simulate_shared_keystrokes("i d o d o g").await;
    cx.shared_state().await.assert_eq("doð¶Ë");
...
async fn test_remap_nested_pineapple(cx: &mut gpui::TestAppContext) {
    ...
    cx.neovim.exec("imap pin ð").await;
    cx.neovim.exec("imap pine ð²").await;
    cx.neovim.exec("imap pineapple ð").await;
