...
impl LineWrapper {
        ...
        iter::from_fn(move || {
            for (ix, c) in char_indices.by_ref() {
                ...
                if Self::is_word_char(c) {
                    ...
                    // CJK may not be space separated, e.g.: `Hello worldä½ å¥½ä¸ç`
    ...
    pub(crate) fn is_word_char(c: char) -> bool {
        ...
        matches!(c,  '/' | ':' | '?' | '&' | '=') ||
        // `â¯` character is special used in Zed, to keep this at the end of the line.
        matches!(c, 'â¯')
    }
...
mod tests {
    ...
    fn test_truncate_multiple_runs() {
        ...
        fn perform_test(
            ...
            assert_eq!(
                wrapper.truncate_line(text.into(), line_width, Some("â¦"), &mut dummy_runs),
                result
            );
    ...
    fn test_is_word_char() {
        ...
        // Latin-1 Supplement
        assert_word("ÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃ");
        // Latin Extended-A
        assert_word("ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ");
        // Latin Extended-B
        assert_word("ÆÆÆÆÆÆÆÆÆÆÆÆÆÆÆÆ");
        // Cyrillic
        assert_word("ÐÐÐÐÐÐÐÐÐÐÐÐÐÐÐÐ");

        // non-word characters
        assert_not_word("ä½ å¥½");
        assert_not_word("ìëíì¸ì");
        assert_not_word("ããã«ã¡ã¯");
        assert_not_word("ððð");
        assert_not_word("()[]{}<>");
    }
