...
mod tests {
    ...
    async fn test_urls(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state(indoc! {"
            Let's test a [complex](https://zed.dev/channel/had-(oops)) caseË.
        "});

        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://zed.dev/channel/had-(Ëoops)) case.
            "});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights::<HoveredLinkState>(indoc! {"
            Let's test a [complex](Â«https://zed.dev/channel/had-(oops)ËÂ») case.
        "});
    ...
    async fn test_urls_at_beginning_of_buffer(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state(indoc! {"https://zed.dev/releases is a cool Ëwebpage."});

        let screen_coord =
            cx.pixel_position(indoc! {"https://zed.dev/relËeases is a cool webpage."});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights::<HoveredLinkState>(
            indoc! {"Â«https://zed.dev/releasesËÂ» is a cool webpage."},
        );
    ...
    async fn test_urls_at_end_of_buffer(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state(indoc! {"A cool Ëwebpage is https://zed.dev/releases"});

        let screen_coord =
            cx.pixel_position(indoc! {"A cool webpage is https://zed.dev/releËases"});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights::<HoveredLinkState>(
            indoc! {"A cool webpage is Â«https://zed.dev/releasesËÂ»"},
        );
