...
pub mod tests {
    ...
    fn test_clip_point(cx: &mut gpui::AppContext) {
        ...
        assert("ËËâ", false, Left, cx);
        assert("ËËâ", true, Left, cx);
        assert("ËËâ", false, Right, cx);
        assert("ËâË", true, Right, cx);
        assert("ËËð", false, Left, cx);
        assert("ËËð", true, Left, cx);
        assert("ËËð", false, Right, cx);
        assert("ËðË", true, Right, cx);
    ...
    fn test_tabs_with_multibyte_chars(cx: &mut gpui::AppContext) {
        ...
        let point = MultiBufferPoint::new(0, "â\t\t".len() as u32);
        let display_point = DisplayPoint::new(DisplayRow(0), "â       ".len() as u32);
        ...
        let point = MultiBufferPoint::new(2, "ðÎ²\t\t".len() as u32);
        let display_point = DisplayPoint::new(DisplayRow(2), "ðÎ²      ".len() as u32);
        assert_eq!(point.to_display_point(&map), display_point);
        assert_eq!(display_point.to_point(&map), point,);

        // Display points inside of expanded tabs
        assert_eq!(
            DisplayPoint::new(DisplayRow(0), "â      ".len() as u32).to_point(&map),
            MultiBufferPoint::new(0, "â\t".len() as u32),
        );
        assert_eq!(
            DisplayPoint::new(DisplayRow(0), "â ".len() as u32).to_point(&map),
            MultiBufferPoint::new(0, "â".len() as u32),
        ...
        assert_eq!(
            map.clip_point(
                DisplayPoint::new(DisplayRow(0), "â".len() as u32 - 1),
                Left
            ),
        ...
        assert_eq!(
            ...
            DisplayPoint::new(DisplayRow(0), "â".len() as u32)
        );
    }
