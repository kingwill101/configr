...
fn test_ime_composition(cx: &mut TestAppContext) {
    ...
    cx.add_window(|cx| {
        ...
        editor.replace_and_mark_text_in_range(Some(0..1), "Ã ", None, cx);
        editor.replace_and_mark_text_in_range(Some(0..1), "Ã¡", None, cx);
        editor.replace_and_mark_text_in_range(Some(0..1), "Ã¤", None, cx);
        ...
        editor.replace_and_mark_text_in_range(Some(4..999), "Ã¨", None, cx);
        ...
        editor.replace_text_in_range(Some(4..999), "Ä", cx);
...
fn test_move_cursor_multibyte(cx: &mut TestAppContext) {
    ...
    _ = view.update(cx, |view, cx| {
        ...
        assert_eq!(view.display_text(cx), "âââ¯â\nabâ¯e\nÎ±Î²â¯Îµ");
        ...
        assert_eq!(
            view.selections.display_ranges(cx),
            &[empty_range(0, "ââ".len())]
        );
        view.move_right(&MoveRight, cx);
        assert_eq!(
            view.selections.display_ranges(cx),
            &[empty_range(0, "âââ¯".len())]
        );
...
fn test_prev_next_word_boundary(cx: &mut TestAppContext) {
    ...
    _ = view.update(cx, |view, cx| {
        ...
        assert_selection_ranges("use std::Â«ËsÂ»tr::{foo, bar}\n\n  {Â«ËbÂ»az.qux()}", view, cx);

        view.select_to_previous_word_start(&SelectToPreviousWordStart, cx);
        assert_selection_ranges("use stdÂ«Ë::sÂ»tr::{foo, bar}\n\n  Â«Ë{bÂ»az.qux()}", view, cx);
...
async fn test_delete(cx: &mut gpui::TestAppContext) {
    ...
    cx.set_state(indoc! {"
        The Ëquick Ëbrown
        fox Â«ËjumÂ»ps over
        the lazy dog
        ËThe quÂ«ick bËÂ»rown"});
    cx.update_editor(|e, cx| e.backspace(&Backspace, cx));
    cx.assert_editor_state("Ëthe lazy dogË");
}
...
fn test_transpose(cx: &mut TestAppContext) {
    ...
    _ = cx.add_window(|cx| {
        let mut editor = build_editor(MultiBuffer::build_simple("ððâ", cx), cx);
        editor.set_style(EditorStyle::default(), cx);
        editor.change_selections(None, cx, |s| s.select_ranges([4..4]));
        editor.transpose(&Default::default(), cx);
        assert_eq!(editor.text(cx), "ððâ");
        assert_eq!(editor.selections.ranges(cx), [8..8]);

        editor.transpose(&Default::default(), cx);
        assert_eq!(editor.text(cx), "ðâð");
...
async fn test_clipboard(cx: &mut gpui::TestAppContext) {
    ...
    cx.set_state("Â«oneâ ËÂ»two Â«three ËÂ»four Â«five ËÂ»six ");
    ...
    cx.assert_editor_state("two oneâ Ëfour three Ësix five Ë");

    // Paste again but with only two cursors. Since the number of cursors doesn't
    // match the number of slices in the clipboard, the entire clipboard text
    // is pasted at each cursor.
    cx.set_state("Ëtwo oneâ four three six five Ë");
    cx.update_editor(|e, cx| {
        e.handle_input("( ", cx);
        e.paste(&Paste, cx);
        e.handle_input(") ", cx);
    ...
    cx.assert_editor_state(
        &([
            "( oneâ ",
            "three ",
            "five ) Ëtwo oneâ four three six five ( oneâ ",
            "three ",
            "five ) Ë",
        ]
        .join("\n")),
...
fn test_split_selection_into_lines(cx: &mut TestAppContext) {
    ...
    _ = view.update(cx, |view, cx| {
        ...
        assert_eq!(view.display_text(cx), "aaâ¯bbb\ncccâ¯eeee\nfffff\nggggg\nâ¯i");
    });
...
async fn test_auto_replace_emoji_shortcode(cx: &mut gpui::TestAppContext) {
    ...
    editor.update(cx, |editor, cx| {
        ...
        editor.handle_input(":", cx);
        assert_eq!(editor.text(cx), "Hello ð".unindent());

        editor.handle_input(" :smile", cx);
        assert_eq!(editor.text(cx), "Hello ð :smile".unindent());

        editor.handle_input(":", cx);
        assert_eq!(editor.text(cx), "Hello ð ð".unindent());

        // Ensure shortcode gets replaced when it is part of a word that only consists of emojis
        editor.handle_input(":wave", cx);
        assert_eq!(editor.text(cx), "Hello ð ð:wave".unindent());

        editor.handle_input(":", cx);
        assert_eq!(editor.text(cx), "Hello ð ðð".unindent());

        editor.handle_input(":1", cx);
        assert_eq!(editor.text(cx), "Hello ð ðð:1".unindent());
        ...
        assert_eq!(editor.text(cx), "Hello ð ðð:1: Test:wave".unindent());
        ...
        assert_eq!(
            editor.text(cx),
            "Hello ð ðð:1: Test:wave: :wave".unindent()
        );
...
async fn test_no_duplicated_completion_requests(cx: &mut gpui::TestAppContext) {
    ...
    cx.condition(|editor, _| editor.context_menu_visible())
        .await;
    cx.assert_editor_state(indoc! {"fn main() { let a = 2.Ë; }"});
    ...
    cx.condition(|editor, _| editor.context_menu_visible())
        .await;
    cx.assert_editor_state(indoc! {"fn main() { let a = 2.SomË; }"});
    assert!(request.next().await.is_some());
    assert!(request.next().await.is_some());
    assert!(request.next().await.is_some());
    request.close();
...
async fn test_completions_with_additional_edits(cx: &mut gpui::TestAppContext) {
    ...
    cx.assert_editor_state(indoc! {"fn main() { let a = 2.Some(2)Ë; }"});
    ...
    .unwrap();
    apply_additional_edits.await.unwrap();
    cx.assert_editor_state(indoc! {"fn main() { let a = Some(2)Ë; }"});
}
...
struct Row10;"#};
    ...
    // Deletion hunks are ephemeral, so it's impossible to place the caret into them â Zed triggers reverts for lines, adjacent to carets and selections.
    assert_hunk_revert(
        indoc! {r#"struct Row;
                   Ëstruct Row2;
