...
///
/// 1. To mark a range of text, surround it with the `Â«` and `Â»` angle brackets,
///    which can be typed on a US keyboard with the `alt-|` and `alt-shift-|` keys.
///
///    ```text
///    foo Â«selected textÂ» bar
///    ```
///
/// 2. To mark a single position in the text, use the `Ë` caron,
///    which can be typed on a US keyboard with the `alt-shift-t` key.
///
///    ```text
///    the cursors are hereË and hereË.
///    ```
///
...
///
///    ```text
///    one Â«ËreversedÂ» selection and one Â«forwardËÂ» selection
///    ```
///
/// Any â¢ characters in the input string will be replaced with spaces. This makes
...
pub fn marked_text_ranges(
    ...
    for (marked_ix, marker) in marked_text.match_indices(&['Â«', 'Â»', 'Ë']) {
        ...
        match marker {
            "Ë" => {
                if current_range_start.is_some() {
                    if current_range_cursor.is_some() {
                        panic!("duplicate point marker 'Ë' at index {marked_ix}");
                    }
            ...
            "Â«" => {
                if current_range_start.is_some() {
                    panic!("unexpected range start marker 'Â«' at index {marked_ix}");
                }
            ...
            "Â»" => {
                let current_range_start = if let Some(start) = current_range_start.take() {
                    ...
                    panic!("unexpected range end marker 'Â»' at index {marked_ix}");
                };
                ...
                if let Some(current_range_cursor) = current_range_cursor.take() {
                    if current_range_cursor == current_range_start {
                        ...
                        panic!("unexpected 'Ë' marker in the middle of a range");
                    }
                } else if ranges_are_directed {
                    panic!("missing 'Ë' marker to indicate range direction");
                }
    ...
    for range in ranges.iter().rev() {
        if indicate_cursors {
            match range.start.cmp(&range.end) {
                Ordering::Less => {
                    marked_text.insert_str(range.end, "ËÂ»");
                    marked_text.insert(range.start, 'Â«');
                }
                Ordering::Equal => {
                    marked_text.insert(range.start, 'Ë');
                }
                Ordering::Greater => {
                    marked_text.insert(range.start, 'Â»');
                    marked_text.insert_str(range.end, "Â«Ë");
                }
            }
        } else {
            marked_text.insert(range.end, 'Â»');
            marked_text.insert(range.start, 'Â«');
        }
    }
    marked_text
...
mod tests {
    ...
    #[test]
    fn test_marked_text() {
        let (text, ranges) = marked_text_ranges("one Â«ËtwoÂ» Â«threeËÂ» Â«ËfourÂ» fiveË six", true);
        ...
        assert_eq!(
            ...
            "one Â«ËtwoÂ» Â«threeËÂ» Â«ËfourÂ» fiveË six"
        );
    }
}
