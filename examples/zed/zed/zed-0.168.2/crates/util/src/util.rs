...
pub fn truncate_and_remove_front(s: &str, max_chars: usize) -> String {
    ...
    match truncation_ix {
        Some(length) => "â¦".to_string() + &s[length..],
        None => s.to_string(),
    }
}
    ...
    impl<T: Rng> Iterator for RandomCharIter<T> {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            match self.rng.gen_range(0..100) {
                ...
                46..=58 => ['ð', 'ð', 'ð', 'ð'].choose(&mut self.rng).copied(),
                // ascii letters
...
/// E.g. "ð¨âð©âð§âð§ð" will return true, but "ð!" will return false.
