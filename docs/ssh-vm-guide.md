...
Each VM is a Docker container running `openssh-server` with the configr binary
embedded. The host machine acts as the **controller** — it uploads config files
and runs `configr apply` on the VMs via SSH.

## Prerequisites

- **Docker** with `docker compose` plugin
- **configr** compiled binary or `dart run` access
