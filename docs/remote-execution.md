...
The DI container selects which transport to use based on the presence of a
`host` key in the connection configuration:
...
8. All `run()`, `putFile()`, `fetchFile()` calls go over the SSH/SFTP session
